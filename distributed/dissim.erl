...
getNode({_,Host,Name,_,_}) -> string:concat(string:concat( Name, "@"),Host).
    ...
    receive
        ...
        {time, Leader, Start, Time, RevCount} ->
            ...
            case deposeCheck(Leader, Center, RevCount, Total) of
                true->
                    writeOut("ID=~w was deposed at t=~w~n",[getID(Center),Time],Master),
                ...
                _ ->
                    case revoltCheck(Leader, Center, Revolted, Time, Start) of
                        true ->
                            writeOut("ID=~w revolted at t=~w~n",[getID(Center),Time],Master),
        ...
        {startClock, Time} ->
            writeOut("ID=~w became leader at t=~w~n",[getID(Center),Time], Master),
